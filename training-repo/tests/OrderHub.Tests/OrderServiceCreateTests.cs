...
public class OrderServiceCreateTests
{
    ...
    [Fact]
    public async Task CreateOrder_GoldCustomer_SnapshotsRawUnitPrice_NoDoubleDiscount()
    {
        using var db = TestSetup.CreateContext();
        var service = TestSetup.CreateOrderService(db);
        ...
        Assert.True(result.Success);
        // 折扣不可在建立訂單時預先套用到單價快照，否則會與 CalculateTotal 重複折扣。
        Assert.Equal(1000m, result.Value!.Items.Single().UnitPriceSnapshot);
        // 金卡 10% 折扣只在 CalculateTotal 套用一次：1000 → 900（而非重複折扣的 810）。
